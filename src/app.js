...
let is_random = store.get('mechvibes-random') || true;
const keyboardpacks = [];
const mousepacks = [];
...
function checkIfAllSoundLoaded(status_display_elem, app_body) {
  ...
  status_display_elem.innerHTML = 'L1ghtingVibes';
...
(function (window, document) {
  window.addEventListener('DOMContentLoaded', async () => {
    ...
    ipcRenderer.on("refresh", () => {
      ...
      app_logo.innerHTML = "L1ghtingVibes";
    })
    ...
    iohook.on('keydown', ({ keycode }) => {
      ...
      app_logo.innerHTML = keycodes[keycode];
      ...
      // Random sounds
      if(randomSounds){
        if(!nonapplicablekeys.includes(keycode)){
